...
jQuery(function ($) {
    ...
    // ie8
    if (!$.browser.msie || $.browser.version > 8) return;
    $('.dd-shapes').css('z-index', 1);
    
    // ie7
    if (!$.browser.msie || $.browser.version > 7) return;
    var textblockTexts = $('.dd-textblock > div');
...
var ThemeLightbox = (function ($) {
    ...
    return (function () {
        ...
        this.init = function (ctrl) {
            $(".dd-lightbox").live("click", { _ctrl: ctrl }, function (e) {
            ...
            $(".dd-lightbox-wrapper .arrow.left:not(.disabled)").live("click", function () {
                move(current - 1);
            });

            $(".dd-lightbox-wrapper .arrow.right:not(.disabled)").live("click", function () {
                move(current + 1);
            });

            $(".dd-lightbox-wrapper .active").live("click", function () {
                move(current + 1);
            });

            $(".dd-lightbox-wrapper .close").live("click", function () {
                close();
            });
        };
        ...
        function show(src) {
            ...
            var img = $('<img class="dd-lightbox-image active" src="' + getFullImgSrc($(src).attr("src")) + '">');
            ...
            bindMouse($(".dd-lightbox-wrapper .arrow").add(img).add(imgContainer));
        }
        ...
        function move(index) {
            ...
            var target = $('<img class="dd-lightbox-image" alt="" src="' + getFullImgSrc($(images[current]).attr("src")) + '" />');
        ...
        function showError(enable) {
            if (enable) {
                $(".dd-lightbox-wrapper").append($('<div class="lightbox-error">Żądana treść nie może zostać wczytana.<br/>Proszę spróbować ponownie później.</div>')
...
jQuery(window).bind("resize", (function ($) {
    ...
    return function () {
        if (typeof responsiveDesign !== "undefined" && responsiveDesign.isResponsive) {
            $("header.dd-header .dd-shapes").children().css("left", "");
            return;
        }
        var sheetWidth = $(".dd-sheet").width();
        var sheetLeft = $(".dd-sheet").offset().left;
        $("header.dd-header .dd-shapes>*, header.dd-header>.dd-textblock, header.dd-header>.dd-headline, header.dd-header>.dd-slogan").each(function () {
            var object = $(this);
...
(function ($) {
    ...
    $(function ($) {
        var links = $('a.dd-rss-tag-icon[href="#"], a.dd-rss-tag-icon[href=""]'),
